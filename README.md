...
Read and export **your own** Viber Desktop (ViberPC) message history on **your own**
Windows computer. Viber stores its local `viber.db` encrypted with SQLCipher; this tool
decrypts it and produces a readable chronological log (time, peer, direction, text) with
full names and numbers from the address book.
...
|---|---|---|---|
| `export` | export from the live connection **(recommended)** | yes | no |
| `log` | log from the export | no | no |
| `all` | `export` + `log` | yes | no |
| `carve` | pull messages from RAM (partial names) | yes | no |
| `hookkey` | capture the SQLCipher key/salt | yes | — |
| `open` | offline SQLCipher open of a copy | no | yes |
| `schema` | print the schema from memory | yes | no |
